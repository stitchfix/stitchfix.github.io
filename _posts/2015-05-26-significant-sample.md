...
Clearly, we want the p-value to be small and the typical threshold used is \\(5\%\\). In other words, if the p-value is less than \\(5\%\\), and the test group experienced a lower event rate than the control group, we conclude that the action worked. The pre-chosen cutoff (\\(5\%\\)) is also referred to as the *significance level* and plays an important role in determining the required sample size. We will explain the significance level in the following section.
